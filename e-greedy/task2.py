...
You need to write code to plot the graphs as required in task2 of the problem statement:
    ...
    - The simulation framework as well as the BernoulliBandit implementation for this task have been separated from the rest of the assignment code and is contained solely in this file. 
      This will be useful in case you would like to collect more information from runs rather than just regret.
"""
...
def regretPlot(algorithm, p1, p2, horizon, label, num_sims = 50):
  regrets = task2(algorithm, horizon, p1, p2, num_sims)
  plt.plot(p2, regrets)
  plt.xlabel('P2'); plt.ylabel('Regrets')
  plt.title(f'{label} using algorithm : {str(algorithm)[14:-2]}')
  plt.savefig(f'./images/{label}_{str(algorithm)[14:-2]}.png', format='png' , bbox_inches='tight')
  plt.close()
  print(f'Done {label} for {str(algorithm)[14:-2]}')
# END EDITING HERE
...
if __name__ == '__main__':
  ...
  algo = UCB
  horizon = 30000
  partA_p1 = np.array([0.9]* (int(0.9/0.05) +1))
  partA_p2 = np.linspace(0, 0.9, int(0.9/0.05)+1)
  regretPlot(algo, partA_p1, partA_p2, horizon, label= 'PartA', num_sims = 50 )
  '''PART B'''
  algo1 = UCB; algo2 = KL_UCB
  partB_p2 = np.linspace(0, 0.9, int(0.9/0.05)+1)
  partB_p1 = partB_p2 + 0.1
  regretPlot(algo1 , partB_p1, partB_p2, horizon,label= 'PartB', num_sims = 50)
  regretPlot(algo2 , partB_p1, partB_p2, horizon, label= 'PartB', num_sims = 50)
